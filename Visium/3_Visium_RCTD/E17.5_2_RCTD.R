...
coi <- E17.5.merge@meta.data %>% dplyr::filter(orig.ident == "E17.5_2")
E17.5.merge <- E17.5.merge[, rownames(coi)]
SpatialDimPlot(E17.5.merge, crop=T, pt.size.factor = 3, label = T, label.box = F,label.size = 3, images = "slice4")

# interactive
SpatialDimPlot(E17.5.merge, interactive = TRUE,  images = "slice4")
coord <- GetTissueCoordinates(object = E17.5.merge_EYFP@images$slice4)
coord["GCGTGTATGTCGTATT-1_2",]
coord["TATTCCGGCAGTCCTA-1_2",]
E17.5.merge <- subset(E17.5.merge, slice4_imagerow < 70, invert = TRUE)
SpatialDimPlot(E17.5.merge, crop=T, pt.size.factor = 3, label = T, label.box = F,label.size = 3,  images = "slice4")

# saveRDS(E17.5.merge, "E17.5_2_cut.rds")
setwd("~/workspace/data/Multiome_Wnt1/Merge_Wnt1/v14/LogNormAll/1st/filtered/Visium_RCTD/250428_RCTD_region/E17.5_2")
E17.5.merge <- readRDS("E17.5_2_cut.rds")
...
# [1] 15729   209
SpatialDimPlot(E17.5.merge_EYFP, crop=T, pt.size.factor = 3, label = T, label.box = F,label.size = 3, images = "slice4")
...
# saveRDS(E17.5.merge_EYFP, "E17.5_2_EYFP_cut.rds")
setwd("~/workspace/data/Multiome_Wnt1/Merge_Wnt1/v14/LogNormAll/1st/filtered/Visium_RCTD/250428_RCTD_region/E17.5_2")
E17.5.merge_EYFP <- readRDS("E17.5_2_EYFP_cut.rds")
...
coi <- E17.5.merge_EYFP@meta.data %>% dplyr::filter(orig.ident == "E17.5_2")
E17.5.merge_EYFP <- E17.5.merge_EYFP[, rownames(coi)]
counts <- E17.5.merge_EYFP[["Spatial"]]$counts
coords <- GetTissueCoordinates(E17.5.merge_EYFP, image = "slice4")
...
setwd("~/workspace/data/Multiome_Wnt1/Merge_Wnt1/v14/LogNormAll/1st/filtered/Visium_RCTD/250428_RCTD_region/E17.5_2")
saveRDS(RCTD, "RCTD_E17.5_2.rds")

E17.5.merge_EYFP <- AddMetaData(E17.5.merge_EYFP, metadata = RCTD_res)

saveRDS(E17.5.merge_EYFP, "E17.5_2_EYFP_RCTD.rds")

setwd("/Users/iwaseakiyasu/Downloads/250425_RCTD/E17.5_2")
E17.5.merge_EYFP <- readRDS("E17.5_2_EYFP_RCTD.rds")
...
SpatialDimPlot(E17.5.merge_EYFP, crop = TRUE, images = "slice4") + theme(aspect.ratio = ratio_E17.5_2)



SpatialFeaturePlot(E17.5.merge_EYFP, features = "21",  pt.size.factor = 3, ncol = 2, crop = TRUE, images = "slice4") +
