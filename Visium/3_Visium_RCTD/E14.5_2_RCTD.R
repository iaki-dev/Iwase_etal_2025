...
coi <- E14.5.merge@meta.data %>% dplyr::filter(orig.ident == "E14.5_2")
E14.5.merge <- E14.5.merge[, rownames(coi)]
SpatialDimPlot(E14.5.merge, crop=T, pt.size.factor = 3, label = T, label.box = F,label.size = 3, images = "slice2")

# interactive
SpatialDimPlot(E14.5.merge, interactive = TRUE,  images = "slice2")

coi <- E14.5.merge@meta.data %>% dplyr::filter(!seurat_clusters %in% c("8", "19", "22"))
E14.5.merge <- E14.5.merge[, rownames(coi)]
...
# saveRDS(E14.5.merge, "E14.5_2_cut.rds")
...
# [1] 15729   148
SpatialDimPlot(E14.5.merge_EYFP, crop=T, pt.size.factor = 3, label = T, label.box = F,label.size = 3, images = "slice2")
...
# saveRDS(E14.5.merge_EYFP, "E14.5_2_EYFP_cut.rds")
setwd("~/workspace/data/Multiome_Wnt1/Merge_Wnt1/v14/LogNormAll/1st/filtered/Visium_RCTD/250428_RCTD_region/E14.5_2")
E14.5.merge_EYFP <- readRDS("E14.5_2_EYFP_cut.rds")
...
coi <- E14.5.merge_EYFP@meta.data %>% dplyr::filter(orig.ident == "E14.5_2")
E14.5.merge_EYFP <- E14.5.merge_EYFP[, rownames(coi)]
counts <- E14.5.merge_EYFP[["Spatial"]]$counts
coords <- GetTissueCoordinates(E14.5.merge_EYFP, image = "slice2")
...
setwd("~/workspace/data/Multiome_Wnt1/Merge_Wnt1/v14/LogNormAll/1st/filtered/Visium_RCTD/250428_RCTD_region/E14.5_2")
saveRDS(RCTD, "RCTD_E14.5_2.rds")

E14.5.merge_EYFP <- AddMetaData(E14.5.merge_EYFP, metadata = RCTD_res)

saveRDS(E14.5.merge_EYFP, "E14.5_2_EYFP_RCTD.rds")
