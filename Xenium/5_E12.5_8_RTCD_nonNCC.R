...
# さらにnonNCCが分布するサンプリングした縦隔領域を抽出する
# つまり、nonNCCが分布しないのはcelltype.fullの17, 32, 38
coi <- xenium.obj@meta.data %>% dplyr::filter(celltype.full != c("17", "32", "38"))
...
ImageDimPlot(xenium.obj, fov = "fov", axes = F, border.color = "white", border.size = 0.01, cols = "polychrome",
  ...
  scale_y_reverse()
setwd("/Users/iwaseakiyasu/workspace/data/Xenium_data/analysis/Wnt1_Cre_E125_8/RCTD/nonNCC")
ggsave("3_ImageDim_celltype.full_nonNCC_Mediastinum.pdf", width = 7, height = 5)
...
# nonNCCに着目する
using.cluster_lineages <- table_lineage %>% dplyr::select(sub.cluster, E11.5_nonNCC, E12.5_nonNCC, total_cells)
sum_col <- using.cluster_lineages %>% dplyr::select(E11.5_nonNCC, E12.5_nonNCC) %>% apply(1, sum)
using.cluster_lineages$sum_col <- sum_col
using.cluster_lineages <- using.cluster_lineages %>% mutate(ratio_nonNCC =  (sum_col / total_cells * 100))
# 5%以上で切る
using.cluster_lineages <- using.cluster_lineages %>% dplyr::filter(ratio_nonNCC > 5)
using.cluster_lineages <- using.cluster_lineages$sub.cluster %>% as.character()
using.cluster_lineages

# E11.5とE12.5 (scMultiome) のうち5%以上のnonNCCを含むデータを解析対象とする
coi <- gexatac_merge@meta.data %>% dplyr::filter(sample %in% c("nonNCC_nonNCC", "E12.5_nonNCC"),
                                                 seurat_clusters %in% using.cluster_lineages)
nonNCC <- gexatac_merge[, rownames(coi)]
nonNCC$sub.cluster <- nonNCC$sub.cluster %>% as.character()
order_factor <- nonNCC$sub.cluster %>% as.numeric() %>% unique() %>% sort()
nonNCC$sub.cluster <- as.factor(nonNCC$sub.cluster)
nonNCC$sub.cluster <- factor(nonNCC$sub.cluster,
                            levels = order_factor)
Idents(nonNCC) <- nonNCC@meta.data$sub.cluster
DimPlot(nonNCC)
counts <- GetAssayData(nonNCC[, rownames(coi)], layer = "RNA", slot = "counts")
# counts@xが整数でないとreferenceがworkしない
# nonNCC_nonNCC, nonNCC_nonNCCがworkしなかったので、これはsoupXのせい
# roundで整数値する
counts@x <- round(counts@x)

# cluster <- as.factor(nonNCC$cluster_lineage)
cluster <- as.factor(nonNCC[, rownames(coi)]$sub.cluster)
names(cluster) <- colnames(nonNCC[, rownames(coi)])
nUMI <- nonNCC[, rownames(coi)]$nCount_RNA
names(nUMI) <- colnames(nonNCC[, rownames(coi)])
nUMI <- colSums(counts)
...
# saveRDS(RCTD, "RCTD_nonNCC_celltype_doublet_nonNCC_Mediastinum.rds")

RCTD <- readRDS("RCTD_nonNCC_celltype_doublet_nonNCC_Mediastinum.rds")
...
ImageDimPlot(xenium.obj, fov =  "fov", group.by = "predicted.celltype", size = 1, border.size = 0.01, cols = "polychrome", dark.background = T) +
  ggtitle("Cell type") +
  # coord_flip() +
  scale_y_reverse()
ggsave("4_decomp_nonNCC_nonNCC.pdf", width = 7, height = 7)
write.table(xenium.obj@meta.data, "4_meta.predicted.celltype.txt", quote = F, sep = "\t", col.names = NA)

# save
saveRDS(xenium.obj, "xenium.obj_nonNCC.rds")
